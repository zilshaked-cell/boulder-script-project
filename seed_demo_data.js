...
function SEED_generateDemoDataForLastTwoMonths() {
  ...
  var currentStart = progressIso
    ? new Date(progressIso)
...
function SEED_generateDemoDataForDateRange(startDateInput, endDateInput) {
  ...
  if (ctx.shiftRows.length > 0) {
    ...
    shiftSheet
      .getRange(startRowShifts, 1, ctx.shiftRows.length, 12)
      .setValues(ctx.shiftRows);
  }
  ...
  if (ctx.requestRows.length > 0) {
    ...
    requestsSheet
      .getRange(startRowReq, 1, ctx.requestRows.length, dataCols)
...
function SEED_newRequest_(opts) {
  ...
  if (layout.COL_DIRECTION)
    row[layout.COL_DIRECTION - 1] = opts.direction || "";
...
function hasSeedTagInRequests_(sheet, startDate, endDate) {
  ...
  while (match) {
    var r = match.getRow();
    var rowVals = sheet
      .getRange(r, 1, 1, Math.max(colFixDate, colTs || colFixDate))
      .getValues()[0];
