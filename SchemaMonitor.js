...
(function () {
  ...
  function sendEmail_(sheetName, prev, current, reason) {
    ...
    try {
      ...
      var body = '';
      body += 'זוהה שינוי במבנה (כותרות/מיקומים) בכרטיסייה: ' + sheetName + '\n';
      ...
      MailApp.sendEmail({
        to: CONFIG.MAIL.TO,
        subject: CONFIG.MAIL.SUBJECT_PREFIX + ' – ' + sheetName + ' (' + (reason || 'change') + ')',
        body: body
      });
    } catch (err) {
  ...
  SCH.onOpen = function (e) { return checkAll_('onOpen'); };
