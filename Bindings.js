function _getLib_() {
  var lib = (typeof ExportGSS !== 'undefined') ? ExportGSS
           : (typeof exportgoogleSS !== 'undefined') ? exportgoogleSS
           : null;
  if (!lib) {
    throw new Error(
...
/** ידיות תפריט → ספרייה */
function gpt_all()        { _getLib_().exportAllSheets(); }
function gpt_selected()   { _getLib_().openExportSheetDialog(); }
function gpt_schema()     { _getLib_().exportSchemaOnlyAll(); }
function gpt_refresh()    { _getLib_().refreshSheetList(); }
function gpt_openFolder() { _getLib_().openSpreadsheetFolder(); }

/** (ה-HTML רץ מהספרייה וקורא לפונקציות בספרייה; העטיפות כאן לא נדרשות אבל לא מזיקות) */
function exportSingleSheet(name) { return _getLib_().exportSingleSheet(name); }
function getSheetList()          { return _getLib_().getSheetList(); }
