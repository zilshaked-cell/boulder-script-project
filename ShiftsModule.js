...
(function () {
  ...
  var CONFIG = {
    ...
    BONUS_SEPARATOR: ',',
    COLS: {
      notes: { candidates: ['הערות למשמרת', 'הערות', 'notes'], fallbackIndex: 1 },
      shiftId: { candidates: ['ID משמרת', 'shiftId', 'Shift ID'], fallbackIndex: 2 },
      timestamp: { candidates: ['חותמת זמן', 'timestamp', 'Timestamp'], fallbackIndex: 3 },
      employeeId: { candidates: ['ID עובד', 'employee_id', 'Employee ID'], fallbackIndex: 4 },
      employeeName: { candidates: ['שם מלא', 'employeeName', 'Employee Name'], fallbackIndex: 5 },
      email: { candidates: ['מייל', 'email', 'Email'], fallbackIndex: null },
      direction: { candidates: ['כניסה / יציאה', 'כיוון', 'direction'], fallbackIndex: 6 },
      fixDate: { candidates: ['תיקון תאריך', 'תאריך', 'date'], fallbackIndex: 7 },
      fixTime: { candidates: ['תיקון שעה', 'שעה', 'time'], fallbackIndex: 8 },
      jobId: { candidates: ['ID סוג עבודה', 'ID סוגי עבודה', 'jobId'], fallbackIndex: 9 },
      jobName: { candidates: ['סוג עבודה', 'סוג העבודה', 'jobName', 'workType'], fallbackIndex: 10 },
      department: { candidates: ['מחלקה', 'department'], fallbackIndex: 11 },
      units: { candidates: ['כמות היחידות', 'יחידות', 'units'], fallbackIndex: 12 },
      status: { candidates: ['סטטוס משמרת', 'סטטוס', 'status'], fallbackIndex: null },
      workDate: { candidates: ['תאריך משמרת', 'Work Date'], fallbackIndex: null },
      startTime: { candidates: ['שעת התחלה', 'Start Time', 'שעת כניסה'], fallbackIndex: null },
      endTime: { candidates: ['שעת סיום', 'End Time', 'שעת יציאה'], fallbackIndex: null },
      bonusIds: { candidates: ['BonusIds', 'Bonus IDs', 'בונוסים'], fallbackIndex: null },
      manualEdited: { candidates: ['ManualEdited', 'Manual Edited', 'עריכה ידנית'], fallbackIndex: null },
      manualNote: { candidates: ['ManualNote', 'הערת מנהל', 'הערת עריכה'], fallbackIndex: null },
      lastUpdatedBySidebar: { candidates: ['LastUpdatedBySidebar', 'עודכן בסיידבר על ידי'], fallbackIndex: null },
      lastUpdatedAt: { candidates: ['LastUpdatedAt', 'עודכן בסיידבר בתאריך', 'SidebarUpdatedAt'], fallbackIndex: null },
      hasIssues: { candidates: ['HasIssues', 'בעיה', 'Issues'], fallbackIndex: null }
    }
  };
  ...
  function getSheet_() {
    ...
    if (!sh) throw new Error('לא נמצאה כרטיסייה בשם "' + CONFIG.SHEET_NAME + '"');
    return sh;
  }
  ...
  function ensureColumnsForWrite_(sheet, headerMap, keys) {
    ...
    if (updated) {
      ...
      sheet.getRange(CONFIG.HEADER_ROW, 1, 1, lastCol).setValues([headers]);
  ...
  function splitBonusIds_(raw) {
    ...
    var str = Array.isArray(raw) ? raw.join(CONFIG.BONUS_SEPARATOR) : String(raw);
    var parts = str.split(/[;,]/);
    var seen = {};
  ...
  function computeHasIssues_(status, notes) {
    ...
    if (s && (s.indexOf('בעיה') !== -1 || s.indexOf('issue') !== -1 || s.indexOf('problem') !== -1)) return true;
    var n = normKey_(notes);
    if (n && (n.indexOf('issue') !== -1 || n.indexOf('בעיה') !== -1 || n.indexOf('fix') !== -1)) return true;
    return false;
  }
  ...
  function readShiftRow_(row, layout) {
    var shift = {
      ...
      employeeName: layout.employeeName ? norm_(row[layout.employeeName - 1]) : '',
      ...
      hasIssues: computeHasIssues_(layout.status ? row[layout.status - 1] : '', layout.notes ? row[layout.notes - 1] : ''),
      isManualEdited: layout.manualEdited ? normalizeBool_(row[layout.manualEdited - 1]) : false,
      rawRow: null
    };
  ...
  function filterShifts_(list, filters) {
    ...
    var statusesFilter = Array.isArray(filters.statuses)
      ...
      : [];
    var statusSet = {};
    statusesFilter.forEach(function (s) { statusSet[s] = true; });
    ...
    return list.filter(function (s) {
      ...
      if (onlyWithBonuses && (!s.bonusIds || s.bonusIds.length === 0)) return false;
      ...
      if (search) {
        var blob = (s.shiftId || '') + ' ' + (s.employeeName || '') + ' ' + (s.employeeId || '') + ' ' + (s.status || '') + ' ' + (s.workType || '') + ' ' + (s.department || '');
  ...
  function list_(filters) {
    ...
    if (lastRow <= CONFIG.HEADER_ROW) {
      return { ok: true, shifts: [], total: 0, returned: 0, hasMore: false, statuses: [] };
    }

    var data = sheet
      .getRange(CONFIG.HEADER_ROW + 1, 1, lastRow - CONFIG.HEADER_ROW, layoutInfo.lastCol)
      .getValues();
    ...
    return {
      ...
      hasMore: hasMore,
      statuses: Object.keys(allStatuses).map(function (k) { return allStatuses[k]; })
    };
  }
  ...
  function findRowByShiftId_(sheet, layout, shiftId) {
    ...
    var range = sheet.getRange(CONFIG.HEADER_ROW + 1, targetCol, lastRow - CONFIG.HEADER_ROW, 1);
  ...
  function updateBonuses_(shiftId, bonusIds) {
    ...
    try {
      ...
      if (layout.bonusIds) sheet.getRange(rowIndex, layout.bonusIds).setValue(bonusStr);
      if (layout.manualEdited) sheet.getRange(rowIndex, layout.manualEdited).setValue(true);
      if (layout.lastUpdatedBySidebar) sheet.getRange(rowIndex, layout.lastUpdatedBySidebar).setValue(Session.getActiveUser().getEmail() || 'sidebar');
      if (layout.lastUpdatedAt) sheet.getRange(rowIndex, layout.lastUpdatedAt).setValue(new Date());
  ...
  function getBonusSheet_() {
    ...
    if (!sh) throw new Error('לא נמצאה כרטיסייה בשם "' + BONUS_CONFIG.SHEET_NAME_OPTIONS + '"');
    return sh;
  }
  ...
  function bonusCols_(map) {
    return {
      ...
      description: map['תיאור בונוס'] || map['Bonus Description'] || map['Description'] || 0,
  ...
  function listAllBonuses_() {
    ...
    var headers = sh.getRange(BONUS_CONFIG.HEADER_ROW, 1, 1, lastCol).getValues()[0];
    var map = buildHeaderMap_(headers);
    var cols = bonusCols_(map);

    var data = sh.getRange(BONUS_CONFIG.HEADER_ROW + 1, 1, lastRow - BONUS_CONFIG.HEADER_ROW, lastCol).getValues();
    var res = [];
    for (var i = 0; i < data.length; i++) {
      var row = data[i];
