...
function handleRequestPost(e) {
  ...
  try {
    ...
    if (!sheet) {
      return jsonResponse({ success: false, error: 'SHEET_NOT_FOUND', sheetName: SHEET_NAME_REQUESTS });
    }
    ...
    if (!COL_REQUEST_ID || !COL_TIMESTAMP) {
      return jsonResponse({
        ...
        details: 'חסרות כותרות חובה (לפחות "ID בקשה" ו-"חותמת זמן") בכרטיסייה "בקשות עובדים".',
      });
    }
    ...
    var status = (data.status !== undefined) ? normalize_(data.status) : 'pending';
    ...
    var shiftNote = (data.shiftNote !== undefined) ? normalize_(data.shiftNote) : '';

    var direction = (data.direction !== undefined) ? normalize_(data.direction) : '';
    ...
    var units = (data.units !== undefined && data.units !== null && data.units !== '') ? Number(data.units) : '';
    ...
    if (existingRow) {
      ...
      if (COL_TIMESTAMP && data.hasOwnProperty('timestamp')) {
        updates.push({ col: COL_TIMESTAMP, val: normalize_(data.timestamp) || now });
      }
      ...
      return jsonResponse({ success: true, requestId: requestId, mode: 'updated', rowIndex: existingRow });
    }
    ...
    return jsonResponse({ success: true, requestId: requestId, mode: 'inserted', rowIndex: newRowIndex });
  } catch (err2) {
    return jsonResponse({ success: false, error: 'UNEXPECTED', details: String(err2) });
  } finally {
